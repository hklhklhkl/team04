...
<BODY>
<FORM id="addForm" action="${pageContext.request.contextPath}/front/study/studyServlet?method=updateStudy" method="post">
<DIV id="add">
<DIV id="close"><BR><A 
href="${pageContext.request.contextPath }/front/study/noteOne.jsp"><INPUT type="button" value="关闭"></A>
		 ...
		 <input type="hidden" value="${study.noteId}" name="noteId"><BR>名字：<INPUT name="noteTitle" type="text" value="${study.noteTitle}"><BR><BR>
			类型：<INPUT name="noteType" type="text" value="${study.noteType}"><BR><BR>手记：<TEXTAREA name="noteContent" rows="11" cols="90">${study.noteContent}</TEXTAREA><BR>
			<BR><INPUT type="submit" value="修改"> <!-- style="float: right;" onclick="chooseSubmit();" type="button" value="上传"> -->
		</DIV></FORM></BODY>
</html>
